...
def setServo(servo, degrees):
    ...
    if (servo>=0) and (servo<=3) and (degrees>= -90) and (degrees<=90):
        ...
        bus.write_byte_data(PCA, SERVOS + servo*4 + 0, int(start) and 0xff)
        bus.write_byte_data(PCA, SERVOS + servo*4 + 1, int(start) >> 8)
        bus.write_byte_data(PCA, SERVOS + servo*4 + 2, int(stop) and 0xff)
        bus.write_byte_data(PCA, SERVOS + servo*4 + 3, int(stop) >> 8)
